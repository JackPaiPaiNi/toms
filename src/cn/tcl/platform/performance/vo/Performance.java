package cn.tcl.platform.performance.vo;

public class Performance {

	private String countryId;
	private String country;
	private String branch;
	private String branchId;
	private int shopId;
	private String shopName;
	
	
	private String customerId;
	private String customerName;
	private String model;
	private String dataDate;
	private int saleQty;
	private double salePrice;
	private String amt;
	private String userId;
	private String userName;
	private int stockQty;
	private int disPlayQty;
	private int classId;
	private int h_qty;
	private double h_price;
	private String h_amt;

	private String remark;

	private String ctime;

	private String id;
	private int type;
	private String targetId;
	
	private int targetQty;
	private String targetAmt;
	private String h_targetAmt;
	private int lineTrager;
	private int ChallengeTarget;
	private String line;
	
	
	
	
public String getH_targetAmt() {
		return h_targetAmt;
	}

	public void setH_targetAmt(String h_targetAmt) {
		this.h_targetAmt = h_targetAmt;
	}

public String getTargetAmt() {
		return targetAmt;
	}

	public void setTargetAmt(String targetAmt) {
		this.targetAmt = targetAmt;
	}

public String getLine() {
		return line;
	}

	public void setLine(String line) {
		this.line = line;
	}

public int getType() {
		return type;
	}

	public void setType(int type) {
		this.type = type;
	}

	public String getTargetId() {
		return targetId;
	}

	public void setTargetId(String targetId) {
		this.targetId = targetId;
	}

	public int getTargetQty() {
		return targetQty;
	}

	public void setTargetQty(int targetQty) {
		this.targetQty = targetQty;
	}

	public int getLineTrager() {
		return lineTrager;
	}

	public void setLineTrager(int lineTrager) {
		this.lineTrager = lineTrager;
	}

	public int getChallengeTarget() {
		return ChallengeTarget;
	}

	public void setChallengeTarget(int challengeTarget) {
		ChallengeTarget = challengeTarget;
	}

public String getCustomerId() {
		return customerId;
	}

	public void setCustomerId(String customerId) {
		this.customerId = customerId;
	}

	public String getCustomerName() {
		return customerName;
	}

	public void setCustomerName(String customerName) {
		this.customerName = customerName;
	}

public String getId() {
		return id;
	}

	public void setId(String id) {
		this.id = id;
	}

private String source;
	
	
	
	

	public String getSource() {
		return source;
	}

	public void setSource(String source) {
		this.source = source;
	}
	
	public int getH_qty() {
		return h_qty;
	}

	public void setH_qty(int h_qty) {
		this.h_qty = h_qty;
	}

	public double getH_price() {
		return h_price;
	}

	public void setH_price(double h_price) {
		this.h_price = h_price;
	}

	public String getH_amt() {
		return h_amt;
	}

	public void setH_amt(String h_amt) {
		this.h_amt = h_amt;
	}

	public String getCtime() {
		return ctime;
	}

	public void setCtime(String ctime) {
		this.ctime = ctime;
	}

	public String getRemark() {
		return remark;
	}

	public void setRemark(String remark) {
		this.remark = remark;
	}

	public int getClassId() {
		return classId;
	}

	public void setClassId(int classId) {
		this.classId = classId;
	}

	public String getBranchId() {
		return branchId;
	}

	public void setBranchId(String branchId) {
		this.branchId = branchId;
	}


	
	public String getAmt() {
		return amt;
	}

	public void setAmt(String amt) {
		this.amt = amt;
	}

	public int getShopId() {
		return shopId;
	}

	public void setShopId(int shopId) {
		this.shopId = shopId;
	}

	public String getBranch() {
		return branch;
	}

	public void setBranch(String branch) {
		this.branch = branch;
	}

	public String getCountryId() {
		return countryId;
	}

	public void setCountryId(String countryId) {
		this.countryId = countryId;
	}

	public String getCountry() {
		return country;
	}

	public void setCountry(String country) {
		this.country = country;
	}

	public String getShopName() {
		return shopName;
	}

	public void setShopName(String shopName) {
		this.shopName = shopName;
	}

	public String getModel() {
		return model;
	}

	public void setModel(String model) {
		this.model = model;
	}

	public String getDataDate() {
		return dataDate;
	}

	public void setDataDate(String dataDate) {
		this.dataDate = dataDate;
	}

	public int getSaleQty() {
		return saleQty;
	}

	public void setSaleQty(int saleQty) {
		this.saleQty = saleQty;
	}

	public double getSalePrice() {
		return salePrice;
	}

	public void setSalePrice(double salePrice) {
		this.salePrice = salePrice;
	}

	public String getUserId() {
		return userId;
	}

	public void setUserId(String userId) {
		this.userId = userId;
	}

	public String getUserName() {
		return userName;
	}

	public void setUserName(String userName) {
		this.userName = userName;
	}

	public int getStockQty() {
		return stockQty;
	}

	public void setStockQty(int stockQty) {
		this.stockQty = stockQty;
	}

	public int getDisPlayQty() {
		return disPlayQty;
	}

	public void setDisPlayQty(int disPlayQty) {
		this.disPlayQty = disPlayQty;
	}

}
