/**
 * fzl
 * Type_target类型 0为导入国家，1为渠道，2为门店
 */
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != "" && data != null) {
				$.ajax({
					dataType : "JSON",
					url : baseUrl + "platform/loadStoreData.action",
					type : "POST",
					data : {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != "" && data != null) {
				var url="";
				if(Type_target=="1"){
					 url=baseUrl + "platform/getChannelTarget.action";
				}else{
					 url=baseUrl + "platform/loadChannelData.action";
				}
				$.ajax({
					dataType : "JSON",
					url : url,
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != "" && data != null) {
				$.ajax({
					dataType : "JSON",
					url : baseUrl + "platform/loadOfficeData.action",
					type : "POST",
					data : {
						"partyId" : obj.rows[0].countryId,"datadate":datadate
					},
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != "" && data != null) {
				$.ajax({
					dataType : "JSON",
					url : baseUrl + "platform/loadRegionData.action",
					type : "POST",
					data : {
						"partyId" : obj.rows[0].countryId,"datadate":datadate
					},
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != "" && data != null) {
				var url="";
				if(Type_target=="1"){
					url=baseUrl + "platform/getBranchTarget.action";
				}else if(Type_target=="2"){
					url=baseUrl + "platform/loadBranchData.action";
				}else{
					url=baseUrl + "platform/getBranchTargetList.action";
				}
				$.ajax({
					dataType : "JSON",
					url : url,
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != "" && data != null) {
				var url="";
				if(Type_target=="1"){
					url=baseUrl + "platform/getSupervisorTarget.action";
				}else{
					url=baseUrl + "platform/loadSupervisorData.action";
				}
				$.ajax({
					dataType : "JSON",
					url : url,
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != "" && data != null) {
				var url="";
				if(Type_target=="1"){
					url=baseUrl + "platform/getSalemanTarget.action";
				}else{
					url=baseUrl + "platform/loadSalesmanData.action";
				}
				$.ajax({
					dataType : "JSON",
					url : url,
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != "" && data != null) {
				var url="";
				if(Type_target=="1"){
					url=baseUrl + "platform/getBusinessTarget.action";
				}else{
					url=baseUrl + "platform/loadManagerData.action";
				}
				$.ajax({
					dataType : "JSON",
					url : url,
...
//导入数据
function importStoreTarget(val){
	//showImportWin(locale("store.import"),baseUrl + "platform/importStore.action");
	showTargetImportWin(locale("excel.target.import"),baseUrl + "platform/importStore.action?targetType=" + val);
}
...
function autoSumChannel(){
        ...
        $('.channel-sum').each(function(){
        	var index="";
        	if(Type_target=='1'){
        		index = $('tr th').index(this)+1; //index(this)是当前文本下标，-3是返回来
        	}else{
        		index = $('tr th').index(this)-3; //index(this)是当前文本下标，-3是返回来
        	}
            var total = 0;
            $(this).parent('tr').parent('thead').next('tbody').children('tr').each(function(){
                var text = $(this).find('td').eq(index).text().replace(/[^0-9.]/ig,"");
...
function autoSumManager(){
    ...
    $('.managers_sum').each(function(){
    	var index = ""; //index(this)是当前文本下标，-31是返回来
    	if(Type_target=="1"){
    		index=$('tr th').index(this)-15;
    	}else{
    		index=$('tr th').index(this)-27;
    	}
        var total = 0;
        $(this).parent('tr').parent('thead').next('tbody').children('tr').each(function(){
