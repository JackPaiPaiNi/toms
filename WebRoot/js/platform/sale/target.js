	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != "" && data != null) {
				$.ajax({
					dataType : "JSON",
					url : baseUrl + "platform/getOBCTVBranchTarget.action",
					type : "POST",
					data : {
...
function importStoreTarget(val){
	...
	showImportWin(locale("excel.target.import"),baseUrl + "platform/importStore.action?targetType=" + val);
}
...
function autoSumBranch(){
    ...
    $('.branch_sum').each(function(){
        ...
        total_objs[index] = total.toFixed(2);//求和保留两位小数
    });
