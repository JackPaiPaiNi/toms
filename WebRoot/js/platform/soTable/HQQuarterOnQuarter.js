...
//	年季月周切换
	...
	$(window).resize(function(){
		var e = $('.rightTabel thead').height();
		$('.leftTabel thead th').css('height',e);
		moveX('YearTotalRight','YearTotalLeft');
	})
});
//表格标题,每个表格加载完成添加
function setSotitle(){
	...
	$('.selctPadTop>p').each(function(){
		$(this).html($($(this).parent().siblings('.tableArea').find('.leftQuarterTabel table thead tr th')[0]).html());
	});
}//表格标题结束
...
function moveX(right,left){
	...
	if(c < d){
		$("#" + left).css("max-height",a - getScrollbarWidth());
	}else{
