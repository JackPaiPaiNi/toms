...
$(function() {
	...
	$("#attachupload").click(function(){
		//setTimeout("refreshProcessBar()", 100);
		refreshProcessBar();
		var options={
				type:"POST",
				url:baseUrl+"updownload/fileUploadAction.action?flag=attach",
				success:function(data){
					$("#hidattach").val(data.path);
				}
		};
...
function refreshProcessBar() {
	$.ajax({
		url:baseUrl+'updownload/stateAction.action',
...
function file_change() {
	...
	if (postfix == "JPG" || postfix == "GIF" || postfix == "PNG"
		...
		alert("您上传的文件类型不被支持，本系统只支持JPG,GIF,PNG,BMP,RAR,ZIP,TXT,MP4文件！");
...
function insertFile() {
	...
	// 新建File表单
	var file_array = form.getElementsByTagName("input");
	var is_null = false;
