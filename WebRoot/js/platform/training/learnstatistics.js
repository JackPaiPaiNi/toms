...
$(function(){
	...
	if(countryId=='304'){
		$("#learnList").datagrid({
			title:jsLocale.get("learn.title"),
			url:baseUrl+"training/loadRussiaLearnStatistics.action",
			pagination:true,
			rownumbers:true,
			singleSelect:true,
			boder:false,
			...
			fit:true,
			columns:[[
			          {field:'partyName',title:jsLocale.get("learn.country"),width:50},
			          {field:'userLogin',title:jsLocale.get("learn.userlogin"),width:30},
			          {field:'userName',title:jsLocale.get("learn.username"),width:40},
			          {field:'userWC',title:jsLocale.get("learn.userwc"),width:50},
			          ...
			          {field:'userWorkNum',title:jsLocale.get("learn.userworknum"),width:30},
			          /*{field:'visitTime',title:jsLocale.get("learn.visit"),width:80,formatter:todate},*/
			          {field:'time',title:jsLocale.get("learn.visit"),width:80,formatter:todate},
			          {field:'courseTittle',title:jsLocale.get("learn.coursetitle"),width:120},
			          {field:'readState',title:jsLocale.get("learn.readState"),width:30,formatter:stateFormatter},
			          {field:'operationScore',title:jsLocale.get("learn.operationScore"),width:30},
			]],
			 toolbar:'#coursetb',
		...
		$("#learnList").datagrid({
			title:jsLocale.get("learn.title"),
			url:baseUrl+"training/loadLearnStatistics.action",
			pagination:true,
			rownumbers:true,
			singleSelect:true,
			boder:false,
...
function exportExcel(){
	...
	var startdate=$(".startdate").val();
	var enddate=$(".enddate").val();
	var url=baseUrl+"training/exportLearnExcel.action?levelOneTypeId="+levelOneTypeId+
...
function exportRusExcel(){
	...
	var startdate=$(".startdate").val();
	var enddate=$(".enddate").val();
	var url=baseUrl+"training/exportRusLearnExcel.action?levelOneTypeId="+levelOneTypeId+
					(levelTwoTypeId?"&levelTwoTypeId="+levelTwoTypeId:"")+
					(levelThreeTypeId?"&levelThreeTypeId="+levelThreeTypeId:"")+
					(region?"&region="+region:"")+
					(country?"&country="+country:"")+
