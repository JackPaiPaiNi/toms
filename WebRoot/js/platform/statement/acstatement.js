...
$(function(){
	...
	//-----------Rangking Of Sales Achieving Rate
	$('#sales_rank').change(function(){
		loadSalesRank(headSelect());
	});
	
	$('#acMachineType').change(function(){
		productAttr();
	});
	
	$('#autoNamesAttr').change(function(){
		selectSizeByFunction();
	});
	
	$('#single').change(function(){
		loadItemSaleInfo($(this).attr('name'));
	});
	
	$("#codeTdate").change(function(){
		loadItemSaleInfo($(this).attr('name'));
	});
...
function acType (){//窗体、分体
	$.ajax({
		url:baseUrl + "acstatement/selectAcMachine.action",
		type:"POST",
		success:function(data){
			var optionStr = "";
			var obj =  eval('(' + data + ')');
			for(var i=0;i<obj.length;i++){
				optionStr += "<option value="+obj[i].value+">"+obj[i].value+"</option>";
			}
			$('#acMachineType').html(optionStr);
			productAttr();
		}
	});
};
...
function productAttr(){//产品属性
	$.ajax({
		url:baseUrl + "acstatement/selectACCatena.action",
		type:"POST",
		data:{'acType':$('#acMachineType').val()},
		success:function(data){
			var optionStr = emptySelectio;
			var obj =  eval('(' + data + ')');
			if(obj != null && obj != ''){
				for(var i=0;i<obj.length;i++){
					optionStr += "<option value="+obj[i].catena+">"+obj[i].catena+"</option>";
				}
			}
...
//--------------------------------------------(AC)通过系列筛选匹数-----------------------------
function selectSizeByFunction(){
	$.ajax({
		url:baseUrl + "acstatement/selectACSize.action",
		type:"POST",
		data:{'catena':$('#autoNamesAttr').val(),'acType':$('#acMachineType').val()},
		success:function(data){
			var obj =  eval('(' + data + ')');
			var optionStr = emptySelectio;
			if(data != '' && data != null && obj != "" && obj != null){
				for(var i=0;i<obj.length;i++){
					optionStr += "<option value="+obj[i].size+">"+obj[i].size+"</option>";
				}
			}
...
function querySizeByAll(){
	$.ajax({
		...
		success:function(data){
			...
			var optionStr = "";
			for(var i=0;i<obj.length;i++){
				optionStr += "<option>"+obj[i].size+"</option>";
			}
			$('#id_size').html(optionStr);
			$('.selectSize').selectpicker({'selectedText': 'cat'});
		}
	});
}
		...
		success:function(data){
			...
			if(data !=  "" && data != null && obj.model != "" && obj.model != null){
				...
				if(initializeSaleCodeModel){
					...
					$('.selectItem').selectpicker({'selectedText': 'cat'});
...
function loadCodeLineChar(name,texts,yearArr,timeArr,showArr){//重点产品折线图
	var myChart = echarts.init($("[name='"+name+"']")[0]); 
	option = {
	    title: {
	        text: texts
...
function loadLineChar(name,texts,yearArr,timeArr,showArr){//折线图
	var myChart = echarts.init($("[name='"+name+"']")[0]); 
	option = {
	    title: {
	        text: texts
...
function loadLineChart(name,texts,yearArr,timeArr,oneArr,twoArr){//加载折线图
	var myChart = echarts.init($("[name='"+name+"']")[0]); 
	option = {
	    title: {
	        text: texts
...
//--------------------区域商店销售进度横圆柱体
function areaSaleNog(byId,nameArray,weekDataArr/*,monthDataArr,quarterDataArr,yearDataArr*/,comment,seriesName){
	var myChart = echarts.init($("[name='"+byId+"']")[0]);
	option = {
	    title : {
	        text: comment,
 ...
 */
	...
	function loadCoreProductsPartyIdJoint(selectInfoObj){
		$.ajax({
			...
			success:function(data){
				...
				coreProductsAreaRequestDate(partyList);
			}
		});
	};
	...
	function coreProductsManagerAreaJoIntParty(o){
		o.userId = "'"+o.manager+"'";
		var	url=baseUrl + "platform/selectPartyIdByUserManager.action";
		$.ajax({type: "post",
			url:url,
			data: o,
			success:function(msg){
				...
				var countryList = '';
	   			if(jsonObj.length > 0){
	   				countryList += "'"+jsonObj[0].partyId+"'";
	 	 			for(var i=1;i<jsonObj.length;i++){
	 	 				countryList += ",'"+jsonObj[i].partyId+"'";
	 	 			}
	   			}else{
	   				countryList = "''";
	   			}
	   			coreProductsAreaRequestDate(countryList);
			}
		});
	}
	
	function coreProductsAreaRequestDate(partyList){
		var o = {};
		o.partyList = partyList;
		o.startTime = Common_date.getTheFirstDay()[0];
		o.endTime = Common_date.getTheFirstDay()[1];
		o.country = getCountryId();
		o.pType = pType;
 ...
 */
	...
	function queryAllSize(jsonObj){
		...
		var allFun = [];
		var tiemModel = $('[data-id=id_size]').attr('title');
		if("Nothing selected" != tiemModel && '' != tiemModel){
			var modelList = tiemModel.split(', ');
			 for(var i = 0;i<modelList.length;i++){
				 var o = {};
				 o.NAME = parseInt(modelList[i]);//parseInt 转int用来比较 '06' == '6'
				 allFun[i] = o;
			 };
		}
 ...
 */
	...
	function selectCountryByUserId(userId){
		...
		o.userId = userId;
		$.ajax({
			url:baseUrl + "platform/selectCountryByUserId.action",
			type:"POST",
			data:o,
			success:function(data){
				var obj =  eval('(' + data + ')');
				var partyList = jointPartyId(obj.rows);
				loadFunctionSaleLine(partyList);
			}
		});
	};
	...
	function loadFunctionSaleLine(partyList){
		var o = {};
		o.partyList = partyList;
		o.startTime = Common_date.getTheFirstDay()[0];
		o.endTime = Common_date.getTheFirstDay()[1];
	 ...
	 */
		...
		function salesProgressJointPartyByArea(selectInfoObj){
			...
			/*$.ajax({
				url:baseUrl + "platform/selectAreaByPartyId.action",
				type:"POST",
				data:{"partyId":selectInfoObj.selectValue,'isCountry':selectInfoObj.isSelectCountry,'isRegion':isRegion},
				success:function(data){
					var obj =  eval('(' + data + ')');
					var partyListStr = jointPartyId(obj.rows);*/
					if($('#area').val() == 'region'){
...
//			salesVolumeGraphicsShow(yAxis_date,seriesList);
	...
	function efficiencySoldAreaRequestDate(partyList,roleStyle){
		var o = {};
		o.countryList = partyList;
		o.roleStyle = roleStyle;
		var urlParagraph = '';
		if(roleStyle == 'Region head'){
	...
	function personnelEfficiencyRoleRealize(o,resultDate){
		o.catena = $('#autoNamesAttr').val();
		o.size = $('#sizeFunction').val();
		o.acType = $('#acMachineType').val();
		o.pType = pType;
		o.level = $("#storeLevel").val();
	...
	function personnelEfficiencyManagerCalculations(o,resultDate){
		...
		var json = JSON.stringify(o);//selectPartyIdByUserManager
...
$(document).ready(function() {
	...
	loadLine();
   // $('#reservationDaily').html(moment().subtract('days', 29).format('MMMM D, YYYY') + ' - ' + moment().format('MMMM D, YYYY'));
   // $('#reservationDaily').daterangepicker();
