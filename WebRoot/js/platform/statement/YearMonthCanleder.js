...
;(function($, window, document,undefined) {
    ...
    SimpleCanleder.prototype = {
        ...
        _buildTitle: function (canlederBox) {
         ...
         _buildBody: function (canlederBox) {
            ...
            var _title_ul_li = ".title ul li";
            if($(canlederBox).attr('id') == 'simpleCanleder_1'){
	            var count = 12;
	            var that = this;
	            for (var i = 0; i < count; i++) {
	            	
	            	if(i>= 0 && i<9){
	            		var $inner = $("<div/>").addClass("inner").text("0"+ (i + 1));
	            	}else{
            ...
            }else if($(canlederBox).attr('id') == 'simpleCanleder_2'){
            	var count = 4;
	            var that = this;
	            for (var i = 0; i < count; i++) {
	                var $inner = $("<div/>").addClass("inner").text("Q"+(i + 1));
	                var $li = $("<li/>").append($inner).click(function () {
	                    var year = canlederBox.find(_title_ul_li).eq(1).find("select").val();
	                    var month = $(this).find("div.inner").text();
	                    that.$element.val(year + "-" + month);
    ...
    $.fn.simpleCanlede = function(options) {
        i++;
        //创建simpleCanleder的实体
